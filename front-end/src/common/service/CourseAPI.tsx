...
export class CourseAPI {
  ...
  public async createNewCourse(course: Course): Promise<void> {
    try {
      await this.axiosInstance.post<undefined>("/courses", course);
  ...
  public async getAllCourse(
    ...
    try {
      ...
      if (this.typeGuardian.isDataPage<Course>(this.serverResponse.data)) {
  ...
  public async deleteCourseByID(courseID: number): Promise<void> {
    try {
      await this.axiosInstance.delete<undefined>(`/courses/${courseID}`);
      return Promise.resolve<undefined>(undefined);
    } catch (apiError: unknown) {
      try {
        this.axiosError = await this.errorHandler.handleApiError(apiError);
  ...
  public async getCourseByName(courseName: any): Promise<AxiosResponse> {
    this.axiosInstanceGetter = new AxiosInstanceGet();
    this.axiosInstance = this.axiosInstanceGetter.getNewInstance();
    try {
      this.serverResponse = await this.axiosInstance.get(
        `/getCourseByName?courseName=${courseName}`
      );
      this.typeGuardian = new TypeGuard();
      if (this.typeGuardian.isAxiosResponse(this.serverResponse)) {
        return this.serverResponse;
  ...
  public async getAllCoursesByCurrentUser(
    userName: any
