...
function renderCourseTypeDropdownList(courseType: CourseType): ReactElement {
  return (
    <option key={courseType.typeID} value={courseType.typeID}>
      {courseType.typeName}
    </option>
  );
}

function renderCourseLevelDropdownList(courseLevel: CourseLevel): ReactElement {
  return (
    <option key={courseLevel.levelID} value={courseLevel.levelID}>
...
export function ManageCoursePage(props: ManageCoursePageProps): ReactElement {
  ...
  let [selectedCourseTypeID, setSelectedCourseTypeID] = useState<number>(0);
  let [courseTypeHolder, setCourseTypeHolder] = useState<CourseType[]>(
    new Array<CourseType>()
  );
  ...
  let [showCreateCourseForm, setShowCreateCourseForm] = useState<boolean>(
    false
  );
  let [courseLevelHolder, setCourseLevelHolder] = useState<CourseLevel[]>(
    new Array<CourseLevel>()
  );
  let [selectedCourseLevelID, setSelectedCourseLevelID] = useState<number>(0);
  let htmlElement:
    | HTMLInputElement
    | HTMLSelectElement
    | HTMLTextAreaElement
  ...
  let [showViewDetailDialog, setShowViewDetailDialog] = useState<boolean>(
    false
  );
  let button: HTMLButtonElement | undefined;
  let courseID: number | undefined;
  let courseSample: Course | undefined;
  let [formattedLastModified, setFormattedLastModified] = useState<string>("");
  let rawDate: Date | undefined;
  let [formattedDateCreated, setFormattedDateCreated] = useState<string>("");
  let [pendingCourseID, setPendingCourseID] = useState<number>(0);
  let [showEditCourseForm, setShowEditCourseForm] = useState<boolean>(false);
  ...
  function openCreateCourseForm(): void {
    ...
    loadCourseTypeDropdownList().catch((error) => {
      console.error(error);
    });
  ...
  async function loadCourseTypeDropdownList(): Promise<void> {
    try {
      updatedCourseTypeHolder = await courseTypeAPI.getAllCourseTypeInTheSystem();
  ...
  async function loadCourseLevelDropdownList(): Promise<void> {
    try {
      updatedCourseLevelHolder = await courseLevelAPI.getAllCourseLevelByTypeID(
  ...
  async function editCourse(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();
    for (i = 0; i < courseTypeHolder.length; i++) {
      courseType = courseTypeHolder[i];
      if (courseType.typeID === selectedCourseTypeID) {
  ...
  async function loadCourseTable(): Promise<void> {
    try {
      courseDataPage = await courseAPI.getAllCourse(pageIndex, pageSize, search);
  ...
  useEffect(() => {
    if (selectedCourseTypeID !== 0) {
      loadCourseLevelDropdownList().catch((error) => {
        console.error(error);
      });
    }
  }, [selectedCourseTypeID]);
  ...
  useEffect(() => {
    if (props.dialogController.dialogIsConfirmed === true) {
      executeCourseDeletion().catch((error) => {
        console.error(error);
      });
  ...
  return (
    <Container fluid={true}>
      ...
      >
        ...
        <Modal.Body>
          <Form
            ...
            onSubmit={(event) => {
              createCourse(event).catch((error) => {
                console.error(error);
              });
            }}
          >
            ...
            <Form.Group controlId="CourseTypeDropdownList">
              ...
              >
                {courseTypeHolder.map((courseType) =>
                  renderCourseTypeDropdownList(courseType)
                )}
              </Form.Control>
            </Form.Group>
            ...
            <Form.Group controlId="CourseLevelDropdownList">
              ...
              >
                {courseLevelHolder.map((courseLevel) =>
                  renderCourseLevelDropdownList(courseLevel)
                )}
              </Form.Control>
            </Form.Group>

            <Form.Group controlId="TuitionFeeField">
              <Form.Label>Tuition Fee (VND):</Form.Label>
              <Form.Control
                type="number"
                autoComplete="on"
                autoFocus={false}
              ...
              />
              <Form.Text className="text-muted">
                format: numbers only and the minimum value is 0 !
              </Form.Text>
            </Form.Group>
          </Form>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="success" type="submit" form="CreateCourseForm">
            Create Course
          </Button>
          <Button variant="outline-secondary" onClick={closeCreateCourseForm}>
            Cancel
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      >
        ...
        <Modal.Body>
          <Form className="pt-2 pr-5 pl-5 pb-0">
            <Form.Row>
              <Form.Group as={Row} controlId="CourseIDInfo">
                <Form.Label column={true} md={5}>
                  + Course ID:
                </Form.Label>
                <Col md={7}>
                  <Form.Control
            ...
            <Form.Row>
              ...
              <Form.Group as={Row} controlId="CourseLevelInfo">
                <Form.Label column={true} md={5}>
                  + Course Level:
                </Form.Label>
                <Col md={7}>
            ...
            <Form.Group as={Row} controlId="TuitionFeeInfo">
              ...
              <Col md={9}>
                <Form.Control
                  plaintext={true}
                  readOnly={true}
                  value={`${course.tuitionFee.toLocaleString()} đ`}
                />
              </Col>
            </Form.Group>
        ...
        <Modal.Footer>
          <Button variant="info" onClick={closeViewDetailDialog}>
            Close
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      >
        ...
        <Modal.Footer>
          <Button variant="success" type="submit" form="EditCourseForm">
            Save
          </Button>
          <Button variant="outline-secondary" onClick={closeEditCourseForm}>
            Cancel
          </Button>
        </Modal.Footer>
      </Modal>
          ...
          <Row className="bg-white">
            <Col>
              <Breadcrumb>
                ...
                <Breadcrumb.Item active>Manage Course</Breadcrumb.Item>
              </Breadcrumb>
              <h1 className="mb-3">
                <span className="mr-3">Manage Course</span>
                <Button
                  variant="success"
                  type="button"
    ...
    <tr key={course.courseID}>
      ...
      <td>
        ...
        <Button
          ...
          value={course.courseID}
          onClick={(event) => {
            openEditCourseForm(event).catch((error) => {
              console.error(error);
            });
          }}
        >
