...
function renderRoleListSection(role: Role): ReactElement {
  ...
  roleNameWithoutPrefix = role.roleName.slice(5);
  return <span key={role.roleID}>{roleNameWithoutPrefix},&nbsp;</span>;
}
...
export function CreateAccountPage(props: CreateAccountPageProps): ReactElement {
  // Variables declaration:
  let [registerFormHolder, setRegisterFormHolder] = useState<RegisterForm[]>(
    []
  );
  ...
  async function handleAcceptRequest(
    ...
    if (newAccountRoleList.length > 0) {
      ...
      props.dialogController.setDialogBody("Please add at least one role.");
      props.dialogController.setDialogType("error");
      props.dialogController.setShowDialog(true);
      return Promise.reject(new Error("Please add at least one role !"));
    }
  }
  ...
  async function executeRequestRejection(): Promise<void> {
    try {
      await registerFormAPI.rejectCreateAccountRequest(userID);
      await loadRegisterFormTable();
      return Promise.resolve<undefined>(undefined);
    } catch (apiError: unknown) {
      if (props.typeGuardian.isAxiosError(apiError)) {
  ...
  function handleReset(): void {
    loadRoleDropdownList().catch((error) => {
      console.error(error);
    });
  ...
  useEffect(() => {
    ...
    loadRegisterFormTable().catch((error) => {
      console.error(error);
    });
  }, []);
  ...
  useEffect(() => {
    if (props.dialogController.dialogIsConfirmed === true) {
      executeRequestRejection().catch((error) => {
        console.error(error);
      });
    ...
    registerFormTable = (
      <tr>
        <td colSpan={6} className="text-center">
          <h5>There are no register-forms in the system to show here</h5>
        </td>
      </tr>
    );
  } else {
    registerFormTable = registerFormHolder.map((registerForm, index) =>
            ...
            <Col>
              ...
              <Form>
                <Form.Group>
                  <Form.Row className="justify-content-md-center">
                    <Form.Label className="mr-2">
                      <h4>{"~"} Add role for the new account:</h4>
                    </Form.Label>
                ...
                <Form.Group>
                  <Form.Row className="justify-content-md-center">
                    <Form.Label className="mr-2">
                      <h4>{">"} The accepted account will have roles:</h4>
                    </Form.Label>
                    {newAccountRoleList.map((role) =>
                      renderRoleListSection(role)
                    )}
                  </Form.Row>
                </Form.Group>
...
function renderRegisterFormTable(
  ...
  return (
    <tr key={registerForm.formID}>
      ...
      <td>{registerForm.phoneNumber}</td>
      ...
      <td>
        <Button
          ...
          onClick={(event) => {
            handleAcceptRequest(event).catch((error) => {
              console.error(error);
            });
          }}
        >
