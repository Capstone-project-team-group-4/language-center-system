...
export class ExaminationSessionAPI {
    ...
    private studentScore: StudentScore | undefined;
    ...
    public async getExamScore (): Promise<StudentScore> {
        try {
            this.serverResponse = await this.axiosInstance.get<unknown> (
                    "/examination-sessions/current/score"
            );
            if (this.typeGuardian.isStudentScore (this.serverResponse.data)){
                this.studentScore = this.serverResponse.data;
                return Promise.resolve<StudentScore> (this.studentScore);
            }
            else {
                throw new Error ("This server response is not valid !");
            }
